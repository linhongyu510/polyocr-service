...
When the input is likely motion-blurred (low Laplacian variance while the model still
returns high-confidence text), the response carries a `suspected_blur` warning. Motion blur
is dangerous precisely because it yields confidently-scored *wrong* text that a caller
otherwise cannot distinguish (see the robustness benchmark below):
...
The floor is configurable via `POLYOCR_BLUR_VARIANCE_FLOOR` (default `45.0`) and is only
evaluated when the model actually returned text — an empty result never warns.
...
| `POLYOCR_BLUR_VARIANCE_FLOOR` | `45.0` | Emit a `suspected_blur` warning when Laplacian variance is below this and text was returned |
...
![Robustness benchmark: exact-match rate by degradation](docs/assets/robustness.png)

> Generated by `python benchmarks/plot_robustness.py` (data from
> [`docs/robustness.md`](docs/robustness.md)). Green is usable (exact ≥ 0.5), red is
> effectively unreadable; the dashed line marks 0.5.
...
a caller cannot distinguish from a correct result. Up to 9px it is unaffected. The service
surfaces this as a `suspected_blur` warning in the OCR response (a Laplacian-variance focus
measure, see the API section) — turning "the caller cannot tell" into "the caller can",
with the floor set by `POLYOCR_BLUR_VARIANCE_FLOOR`.
